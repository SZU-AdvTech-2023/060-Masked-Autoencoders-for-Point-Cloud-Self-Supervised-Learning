...
from modules.shared_transformer import SharedTransformer,SA_Layer
...
def calculate_cosine_similarity_matrix(h_emb, eps=1e-8):
    ...
    # h_emb (N, M)
    # normalize
    a_n = h_emb.norm(dim=3).unsqueeze(3)
    a_norm = h_emb / torch.max(a_n, eps * torch.ones_like(a_n))

    # cosine similarity matrix
    sim_matrix = torch.einsum('aebc,aecd->aebd', a_norm, a_norm.transpose(2,3))
    return sim_matrix
def calculate_cosine_similarity_matrix_4d(h_emb, eps=1e-8):
    r'''
...
def calculate_cosine_similarity_matrix_3d(h_emb, eps=1e-8):
    r'''
...
class BoxAttention(nn.Module):
    def __init__(self, dim, box_size, num_heads, attn_drop=0., proj_drop=0.):
        ...
        self.kv_conv=nn.Linear(dim,dim*2,bias=False)
    ...
    def forward(self, x, mask=None):
        ...
        #print(x.size())
        with torch.no_grad():
            similarity=calculate_cosine_similarity_matrix_3d(x)
        score=self.softmax(similarity)
        cosine_features=torch.einsum('acd,ade->ace',score,x)
        q = self.q_conv(x).reshape(B_, N, self.num_heads, C // self.num_heads).permute(0,2,1,3)
        kv = self.kv_conv(cosine_features).reshape(B_, N, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        k,v=kv[0],kv[1]
        ...
        #print(q.size())
           ...
           #similarity=1-torch.cosine_similarity(q.unsqueeze(3),q.unsqueeze(2),dim=-1)
        #print(attn.size())
...
class Voxel_Transformer(nn.Module):
    def __init__(self, out_channels, depth,resolution,boxsize,mlp_dims,drop_path1,drop_path2):
        super().__init__()
        self.shift = None
        self.depth = depth
...
class PVTConv(nn.Module):
    ...
    def forward(self, inputs):
        ...
        rel_pos =rel_pos.pow(2).sum(dim=-1).sqrt()

        #print(features.size())
        #with torch.no_grad():
         #   similarity=1-torch.cosine_similarity(features.permute(0,2,1).unsqueeze(2),features.permute(0,2,1).unsqueeze(1),dim=-1) 
        #torch.cuda.empty_cache()
        #print(similarity.size())
        #similarity=rel_pos+similarity
        fused_features = voxel_features + self.point_features(features,voxel_features,rel_pos)
