...
class SharedTransformer(nn.Module):
    ...
    def forward(self, inputs,voxel_features, rel_pos):
        x = F.relu(self.bn1(self.conv1(inputs)))
        x1 = self.sa1(x,voxel_features, rel_pos)
...
class SA_Layer(nn.Module):
    def __init__(self, channels):
        ...
        #self.paconv=PAConv(channels,channels,16,3)
    def forward(self, x,voxel_features, rel_pos):
        n = x.shape[2]
        pos = torch.randn(1, n, 3).cuda(0)
        x_q = self.q_conv(voxel_features).permute(0, 2, 1) # b, n, c 
