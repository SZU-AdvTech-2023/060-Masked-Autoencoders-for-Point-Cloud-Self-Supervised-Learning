...
parser.add_argument('--config', default='cfgs/config_ssn_face_id_siamese.yaml', type=str)

NUM_REPEAT = 10
G_BATCH_SIZE=16 # G_BATCH_SIZE * n <= total samples
THRESHOLD = 0.76 # 0.79 FAR is 0.001

def verification(args, config):
    ...
    test_dataset_1 = KinectLQ_eval(num_points = args.num_points, root = args.data_root, transforms=test_transforms
    ...
    with torch.no_grad():
        ...
        for i in np.arange(0, gallery_num//G_BATCH_SIZE + 1):
            ...
            if i < gallery_num//G_BATCH_SIZE:
                ...
                #g_points=g_points.cuda()
                #print(g_points.size())
                #summary(model,(7000,6))
                #flops,params=profile(model.module,inputs=(g_points,))
